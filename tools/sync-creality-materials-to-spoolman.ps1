...
<#
.SYNOPSIS
  Sync Creality material_database.json entries into Spoolman (vendors + filament types).

.DESCRIPTION
  - Reads a Creality-style material_database.json (commonly root.result.list).
  - Extracts vendor/material/name/color/density/diameter (+ optional temps/price).
  - Ensures Spoolman Vendor exists.
  - Ensures Spoolman Filament exists (matched primarily by article_number = "Creality:<base.id>").
  - Creates missing entities, and optionally patches existing ones.
...
function Build-CrealityRecord([AllowNull()][object]$Entry) {
  ...
  $colorHex   = Normalize-HexColor (First-NonEmptyString @(
      (Get-Prop $kv "default_filament_colour"),
      (Get-FirstString (Get-Prop $base "colors")),
      (Get-Prop $Entry "color_hex"),
      (Get-Prop $Entry "color")
  ))
...
function Build-DesiredFilamentPayload([pscustomobject]$rec, [int]$vendorId) {
  ...
  if ($rec.Name)     { $payload['name']     = (Trunc $rec.Name 64) }
...
function Diff-Filament([object]$existing, [hashtable]$desired) {
  ...
  foreach ($k in @("name","material","color_hex","article_number")) {
...
# Index filaments by article_number + by computed key
$filamentByArticle = @{}
$filamentByKey = @{}
...
foreach ($rec in $records) {
  ...
  if (-not $vendorByName.ContainsKey($vk)) {
    ...
    if ($DryRun) {
      ...
      $newVendor = Invoke-SpoolmanRest -ApiBase $apiBase -Method "POST" -Path "/vendor" -Body $payload -Query $null
  ...
  if ($article) {
    ...
    if ($ak -and $filamentByArticle.ContainsKey($ak)) { $existing = $filamentByArticle[$ak] }
  }

  if ($null -eq $existing) {
    $key = Compute-FilamentKey -vendorId $vendorId -name ($desired["name"]) -material ($desired["material"]) -colorHex ($desired["color_hex"])
    if ($key -and $filamentByKey.ContainsKey($key)) { $existing = $filamentByKey[$key] }
  }
  ...
  if ($existing) {
    ...
    if ($diff.Count -gt 0) {
      ...
      elseif ($DryRun) {
        ...
        $updated = Invoke-SpoolmanRest -ApiBase $apiBase -Method "PATCH" -Path ("/filament/{0}" -f $fid) -Body $diff -Query $null
        $updatedFilaments++
    ...
    if ($DryRun) {
      Write-Host ("[DryRun] Would create filament: vendor={0} name={1} material={2} color={3} article={4}" -f
        $rec.VendorName,
      ...
      $created = Invoke-SpoolmanRest -ApiBase $apiBase -Method "POST" -Path "/filament" -Body $desired -Query $null
      $createdFilaments++
